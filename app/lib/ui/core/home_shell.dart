...
class _HomeShellState extends State<HomeShell> {
  ...
  /// Promote a card's notes from the side column to a fullscreen overlay. The
  /// overlay's toggle restores it (see [_exitCardFullscreen]).
  void _enterCardFullscreen(String cardId) {
    if (!mounted) return;
    setState(() {
      ...
      _presenterScreen = ReaderScreen(
        ...
        cardId: cardId,
        fullscreen: true,
        onToggleFullscreen: _exitCardFullscreen,
      );
    });
  }

  /// Restore the fullscreen notes back to the library side column (the card
